...
from liris_data_helper import wav2fbank
import pandas as pd
...
class VideoFrameDataset(torch.utils.data.Dataset):
    ...
    Args:imagefile_template
        root_path: The root path in which video folders lie.
                   this is ROOT_DATA from the description above.imagefile_template
    ...
    def _load_image(self, directory: str, idx: int) -> Image.Image:
        ...
        if os.path.exists(os.path.join(directory, str(idx)+'.bmp')):
            
            return Image.open(os.path.join(directory, str(idx)+'.bmp')).convert('RGB').resize((224,224))
        else:
            return Image.new('RGB', (224, 224))
    
    def _load_audio(self, directory: str) -> 'torch.Tensor':
        a = directory.split('/')
        if self.test_mode:
            audio_path = '/'+a[1]+'/'+a[2]+ '/'+ a[3]+ '/'+ a[4]+ '/' + a[5]+ '/' + a[6]+ '/test-audio/'+a[8]+ '/%d.wav'
        else:
            audio_path = '/'+a[1]+'/'+a[2]+ '/'+ a[3]+ '/'+ a[4]+ '/' + a[5]+ '/' + a[6]+ '/audio/'+a[8]+ '/%d.wav'
    ...
    def _get_start_indices(self, record: VideoRecord) -> 'np.ndarray[int]':
        ...
        # randomly sample start indices that are approximately evenly spread across the video frames.
            ...
            #print('max ',max_valid_start_index)
            max_valid_start_index = 1
    ...
    def _get(self, record: VideoRecord, frame_start_indices: 'np.ndarray[int]') -> Union[
        ...
        for start_index in frame_start_indices:
            ...
            for _ in range(self.frames_per_segment):
                ...
                #print('num frames',record.num_frames,'end ', record.end_frame,'num seg',self.num_segments,frame_index)

                if frame_index < record.end_frame:
                    frame_index += 1

                if self.test_mode == False:
                    df = pd.read_csv('/media/DATA/admin-edge/Documents/safaa-project/data/domain-labels.csv',sep ='\t')
                    label_d = (np.array(df.loc[df['name'] ==  str(record.path.split('/')[-1])]['movie_id']))[0]
                images.append(image)

        if self.transform is not None:
            images = self.transform(images)
        audio = self._load_audio(record.path)
        #print(images.shape,audio.shape)
        if self.test_mode:
            return images,audio, record.label
        else:
            return images,audio, record.label, label_d
